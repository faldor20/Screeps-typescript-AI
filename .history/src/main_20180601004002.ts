...
function mainLoop() {
  ...
  for (const name in Memory.creeps) {
    if (!(name in Game.creeps)) {
      ...
      if(creepMemory.worker.targetobjectID!="") spawncreep("Sp1", creepMemory.role, creepMemory.worker.targetobjectID);
        // }
      }
  ...
  for (let name in Game.rooms) {
    ...
    if (room.energyAvailable == 300) {
      if (Memory.upgraderCount<3) {
        Game.spawns["Sp1"].spawnCreep([WORK, MOVE, CARRY,WORK], "Upgrader1", {
          memory: { role: "upgrader" }
        });
        Memory.upgraderCount++;
      } else if (Memory.builderCount<3) {
        Game.spawns["Sp1"].spawnCreep([WORK, MOVE, CARRY,WORK], "Builder"+Game.creeps, {
          memory: { role: "builder" }
        });
        Memory.buildercount++;
      }
       else if (room.memory.harvestPoints.free.length>0) {
