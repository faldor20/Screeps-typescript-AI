...
export function run(creep: Creep) {
  if (creep.carry.energy < creep.carryCapacity) {
    let source: Source | null = Game.getObjectById(creep.memory.harvester.sourcetargetID)
    if (source) {
...
function assignHarvester(creep:Creep) {
  let source: Source | null = Game.getObjectById(creep.room.memory.jobs.harvester.free.pop());
  if (source) {
    creep.room.memory.jobs.harvester.taken.push(source.id);
    if(creep.memory.worker)creep.memory.worker.targetobjectID = source.id;
  }
  else
  {
